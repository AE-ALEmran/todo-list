def get_store(filepath):
    with open(filepath, 'r') as file_local:
        store_local = file_local.readlines()
    return store_local

#use def function to define repeted code in the program.
def write_store(filepath, store_arg):
...
while True:
    ...
    if user_choice.startswith('add'):
        ...
        store = get_store(".idea/store.txt")                           # replace function

        store.append(todo + '\n')

        write_store(".idea/store.txt", store)
    elif user_choice.startswith('show'):

        store = get_store(".idea/store.txt")
